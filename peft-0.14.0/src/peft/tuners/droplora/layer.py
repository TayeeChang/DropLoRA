...
from .config import DropLoraConfig
...
class LearnablePruning(nn.Module):
    def __init__(self, dim, temperature=1.0, eps=1e-8):
        super().__init__()
        self.dim = dim
        self.temperature = temperature
        self.eps = eps
        self.logits = nn.Parameter(torch.zeros(dim), requires_grad=True)

    def forward(self, x):
        if self.training:
            mask_prob = torch.sigmoid(self.logits)  # 保留概率
            mask_hard = (mask_prob >= 0.5).to(self.logits.dtype)
            mask = (mask_hard - mask_prob).detach() + mask_prob  # STE技巧
            p = mask_hard.sum() / mask_hard.size()[0]
            # 缩放补偿（确保期望一致）
            scale_factor = 1.0 / (p.detach() + self.eps)
    ...
    def get_mask(self):
        mask_prob = torch.sigmoid(self.logits)
        mask_hard = (mask_prob >= 0.5).to(self.logits.dtype)
        return mask_hard


class DropLoraLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B", "lora_inner_dropout")
    # All names of other parameters that may contain adapter-related parameters
    other_param_names = ("r", "lora_alpha", "scaling", "lora_dropout", "lora_inner_dropout")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora_inner_dropout = nn.ModuleDict({})
    ...
    def update_layer(
        ...
        if not dynamic_pruning:
            self.lora_inner_dropout[adapter_name] = nn.Dropout(p=inner_dropout)
        else:
            self.lora_inner_dropout[adapter_name] = LearnablePruning(r)
        ...
        if isinstance(init_lora_weights, str) and init_lora_weights.startswith("pissa"):
            ...
            with gather_params_ctx(self.get_base_layer().weight):
                self.milora_init(adapter_name, init_lora_weights)
    ...
    def milora_init(self, adapter_name, init_lora_weights):
        weight = self.get_base_layer().weight
        dtype = weight.dtype
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
            raise TypeError(
        ...
        if init_lora_weights == "milora":
            ...
            Vr, Sr, Ur = svd_lowrank(
                weight.data, min(weight.data.shape), niter=int(init_lora_weights.split("_niter_")[-1])
            )
            Sr = Sr[-self.r[adapter_name]:]
            Vr = Vr[:, -self.r[adapter_name]:]
            Ur = Ur[-self.r[adapter_name]:]
            Sr /= self.scaling[adapter_name]
            Uhr = Ur.t()
        else:
            raise ValueError(
                f"init_lora_weights should be 'milora' or 'milora_niter_[number of iters]', got {init_lora_weights} instead."
            )
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            inner_dropout = self.lora_inner_dropout[active_adapter]
            scaling = self.scaling[active_adapter]

            # getting the sub-batch, passing it to LoRA layers and updating the corresponding indices of the linear
            # layer output
            sub_batch = x[sub_batch_indices_list[i]].to(lora_A.weight.dtype)
            lora_output = lora_B(inner_dropout(lora_A(dropout(sub_batch)))) * scaling
...
class Linear(nn.Module, DropLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        DropLoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if isinstance(self.lora_inner_dropout[adapter], LearnablePruning):
            mask = self.lora_inner_dropout[adapter].get_mask()
            output_tensor = transpose((weight_B * mask) @ (weight_A.T * mask).T, self.fan_in_fan_out) * self.scaling[adapter]
        else:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    result = result + lora_B(inner_dropout(lora_A(dropout(x)))) * scaling
...
class Embedding(nn.Module, DropLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        DropLoraLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, DropLoraLayer):
