...
from .layer import Conv2d, DropLoraLayer, dispatch_default
...
class DropLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: DropLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: DropLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        kwargs = {
            ...
            "inner_dropout": lora_config.inner_dropout,
            "dynamic_pruning": lora_config.dynamic_pruning,
        ...
        if isinstance(target, DropLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, DropLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, DropLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, DropLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, DropLoraLayer) or isinstance(module, ModulesToSaveWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, DropLoraLayer):
