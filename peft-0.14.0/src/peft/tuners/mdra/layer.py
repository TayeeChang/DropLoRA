...
class VeraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("vera_lambda_b", "vera_lambda_d", "vera_A", "vera_B")
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        self.vera_A = nn.ParameterDict({})
        self.vera_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        self.vera_A[adapter_name] = nn.Parameter(torch.ones(r, self.in_features), requires_grad=False)
        self.vera_B[adapter_name] = nn.Parameter(torch.ones(self.out_features, r), requires_grad=False)

        if init_weights:
            self.reset_vera_parameters(adapter_name, init_weights, d_initial=d_initial)

        self._move_adapter_to_device_of_base_layer(adapter_name)
        self.set_adapter(self.active_adapters)

    def reset_vera_parameters(self, adapter_name, init_weights, d_initial: float = 0.1):
        with torch.no_grad():
            nn.init.ones_(self.vera_lambda_d[adapter_name]).fill_(d_initial)
            nn.init.ones_(self.vera_lambda_b[adapter_name])
            if init_weights.startswith("pissa"):
                self.pissa_init(adapter_name, init_weights)
            elif init_weights.startswith("milora"):
                self.milora_init(adapter_name, init_weights)
            else:
                raise ValueError("init_weights must be \"pissa\" or \"milora\" ")


    def pick_n_init(self, adapter_name, init_weights):
        ...
        ##
        ##
        indices = torch.randperm(min(weight.data.shape))[:self.r[adapter_name]]
        ##
        ##
        if init_weights == "pissa":
            ...
            Uhr = Uh[indices]
        elif len(init_weights.split("_niter_")) == 2:
            Vr, Sr, Ur = svd_lowrank(
                weight.data, self.r[adapter_name], niter=int(init_weights.split("_niter_")[-1])
            )
            # Sr /= self.scaling[adapter_name]
            Uhr = Ur.t()
        else:
            raise ValueError(
                f"init_lora_weights should be 'pissa' or 'pissa_niter_[number of iters]', got {init_weights} instead."
            )

        lora_A = torch.diag(torch.sqrt(Sr)) @ Uhr
        lora_B = Vr @ torch.diag(torch.sqrt(Sr))
        self.vera_A[adapter_name].data = lora_A
        self.vera_B[adapter_name].data = lora_B
        weight = weight.data - lora_B @ lora_A
        weight = transpose(weight.to(dtype), self.fan_in_fan_out)
        self.get_base_layer().weight.data = weight

    def mdra_init(self, adapter_name, init_weights):
        weight = self.get_base_layer().weight + self.get_delta_weight(adapter_name)
        dtype = weight.dtype
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
            raise TypeError(
                "Please initialize PiSSA under float32, float16, or bfloat16. "
    ...
    def milora_init(self, adapter_name, init_weights):
        weight = self.get_base_layer().weight
        dtype = weight.dtype
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
            raise TypeError(
        ...
        if init_weights == "milora":
            ...
            Vr, Sr, Ur = svd_lowrank(
                weight.data, min(weight.data.shape), niter=int(init_weights.split("_niter_")[-1])
            )
            # Sr /= self.scaling[adapter_name]
            ...
            raise ValueError(
                f"init_lora_weights should be 'milora' or 'milora_niter_[number of iters]', got {init_weights} instead."
            )
...
class Linear(nn.Linear, VeraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, r, vera_dropout, init_weights, d_initial=d_initial)
