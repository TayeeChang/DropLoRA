...
def prepare_adapter(args: TrainArguments, model, *, template=None, train_dataset=None, task_type=None):
    from swift.tuners import (AdaLoraConfig, AdapterConfig, BOFTConfig, LLaMAProConfig, LongLoRAModelType, LoraConfig,
                              LoRAConfig, ReftConfig, Swift, VeraConfig, MdraConfig, DropLoraConfig)
    ...
    if args.train_type in ('lora', 'longlora'):
    ...
    elif args.train_type == 'mdra':
        mdra_config = MdraConfig(
            task_type=task_type,
            r=args.mdra_rank,
            target_modules=target_modules,
            vera_dropout=args.mdra_dropout,
            d_initial=args.mdra_d_initial,
            init_weights=args.mdra_init_weights,
            modules_to_save=args.modules_to_save,
        )
        model = Swift.prepare_model(model, mdra_config)
        logger.info(f'mdra_config: {mdra_config}')
    elif args.train_type == "droplora":
        from peft import DropLoraConfig
        droplora_kwargs = {
        'r': args.lora_rank,
        'target_modules': target_modules,
        'lora_alpha': args.lora_alpha,
        'lora_dropout': args.lora_dropout,
        'inner_dropout': args.droplora_inner_dropout,
        'dynamic_pruning': args.droplora_dynamic_pruning,
        'bias': args.lora_bias,
        'modules_to_save': modules_to_save,
        'use_rslora': args.use_rslora,
        'use_dora': args.use_dora,
        ...
        droplora_config = DropLoraConfig(task_type=task_type, **droplora_kwargs)
        model = Swift.prepare_model(model, droplora_config)
        logger.info(f'droplora_config: {droplora_config}')
